...
- **Claret 500** (`#c1365a`): the canonical CTA fill (2026-09-11, previously Claret 600). 5.33:1 on paper — still clears AA for white button text, and is what `cta-primary` now aliases so the header/home CTA and every other page's primary button render identically. Also used for focus rings (`ring-rose-500`).
- **Claret 600** (`#a12049`): CTA hover (`cta-primary-hover` aliases this too). 7.45:1 on paper, 7.11:1 on canvas.
- **Claret 700** (`#840e38`) / **Claret 800** (`#670228`): reserved for a CTA sitting directly on Chula Pink chrome, one/two stops deeper (see the Chrome Step rule below) — not currently in use now that the header sits on Paper rather than Chula Pink chrome (see Navigation).
- **Claret 200** (`#fbbfc6`): input focus-ring tint (`ring-rose-200`) and `::selection` background.

**Why 500, not 600 (2026-09-11).** A 2026-09-06 pass ("Crisp Paper & Deep Berry," see `globals.css`) gave the header/home CTA its own darker, undocumented color (`#9b2c4f`) and gave the footer a near-black wine fill (`#5c1f30`), separate from the Claret 600 every other page's buttons used. Three unrelated dark reds read as inconsistent "oxblood" tones depending on which page you were on — direct visual feedback confirmed it. Rather than pick a fourth color, every primary button (including the ones that already used Claret 600) moved one step lighter to Claret 500, and `cta-primary`/`cta-primary-hover` now alias Claret 500/600 instead of their own pair. One shade, everywhere.
...
**The One Brand Color Rule.** Chula Pink appears once, as header chrome, at full strength (never tinted or diluted), and nowhere else. Anything that needs to look pink for an _interactive_ reason uses Claret instead — the two never substitute for each other.
...
- **Home page:** four stacked regions, top to bottom — the `SplitHero` first viewport, a four-item Capability Grid, a Live Zone Availability section on a `bg-canvas-subtle` tinted band, and the Site Footer (restored 2026-09-11 to this pre-Bento structure; see Components below). Horizontal padding steps `px-6 sm:px-10 lg:px-16 xl:px-24` and is shared by every region so their left edges align down the page.
...
- **Primary** (modal `Continue`, admin-login submit, hero CTA, Reserve table): Claret 500 background (`#c1365a`) → Claret 600 on hover (`#a12049`), white text, `px-3 py-2 text-sm font-medium`. On Chula Pink chrome this steps two stops deeper — see the Chrome Step Rule.
...
The home page's value-proposition section: four capability cards in a `1 → 2 (min-[860px])` column grid, sitting immediately after the hero. One card is "featured" (currently "Real-time booking status") — a 2px Chula Pink border, a `bg-chula-pink/10` icon well, and a Chula Pink icon, versus the other three's `border-hairline` + white icon well + `text-cta-primary` icon. Featuring exactly one card this way is the section's only accent; the rest stay visually equal.

- **Card:** `rounded-xl`, `border` (2px Chula Pink when featured, else `border-hairline`), `bg-paper`, `p-6`, `shadow-sm` — the one place outside cards/modals this deck uses a shadow (a deliberate exception, not drift toward ambient shadow use elsewhere).
- **Heading level:** cards use `<h3>` under a `SectionHeading` (eyebrow "What CULibSpace does" + title + subtitle) that provides the section's one visible `<h2>`.

**Restored 2026-09-11.** A six-item asymmetric "Bento" version (one tile elevated to `lg:col-span-2 lg:row-span-2` with `ZoneAvailabilityGraphic` embedded inside it, no shadows, `border-gray-200`) existed only as an uncommitted local change — it was never the version merged to `main` and was reverted per direct feedback back to this four-card-plus-separate-zone-section structure, last committed at `a68824c`. Don't reintroduce the Bento merge of capabilities-and-availability into one tile without a new decision; the two are separate sections now (see Zone Availability Graphic below).

### Zone Availability Graphic

Lives in its own card inside the **Live zone availability** section (a `bg-canvas-subtle` tinted band below the Capability Grid), alongside a `SignInUnlockCard` in the same `1 → 2 (min-[860px])` grid — not embedded inside a capability tile. The first place the product's real-time claim is shown rather than described: three zone rows (Silent, Group, Common), each a label, a `rounded-full` fill bar, and an explicit `{open} of {total} open` count.
...
- **Surface:** a soft Claret 100 pink tint (`bg-footer-bg`, `#fedfe2`) with a `border-t border-rose-200` rule — deliberately, visibly pink, revised 2026-09-11 after both a near-black wine fill (2026-09-06) and a plain-white/Paper attempt read as wrong: wine was "too much," and Paper on the next pass dropped the pink identity the footer is meant to carry entirely. No shadow, no full-bleed saturated color.
...
- **Type roles:** `text-sm font-semibold text-ink` column headings, `text-sm text-ink/70` links and contact lines, `text-xs text-ink/50` copyright. Links are ghost-style: no fill, Chula Pink text with underline on hover — the footer's one deliberate use of Chula Pink as an _interactive_ color, kept as the incumbent choice rather than moved to Claret when the footer's dark fill was reverted on 2026-09-11.
