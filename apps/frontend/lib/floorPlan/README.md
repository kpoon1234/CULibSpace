...
Override the path with `NEXT_PUBLIC_FLOORPLAN_PATH` (default `/api/layout`).
...
// RawLayoutZone
{
  ...
  "bounds": { "x": 0, "y": 0, "width": 1120, "height": 680 }, // optional
  "tables": [
    {
      "tableId": 101,
      "numberOfSeat": 4,
...
| `date` + `timeSlot` **or** `startDateTime` + `endDateTime` | the window `status` is computed for            |

`toQueryString()` in `client.ts` serialises all of these.
...
useFloorPlan(floorId, { filter, serverFilter? })   // the only thing components call
  └─ fetchLayout(query)          // GET /api/layout, polled every STATUS_POLL_MS (20s)
...
- **Amenity filtering is client-side by default.** `useFloorPlan` sends only the
  **time window** to `/api/layout`; `plugCap` / `hasTvScreen` / `minSeats` are
...
`lib/auth.ts`) and make sure `GET /api/layout` responds. Nothing in
